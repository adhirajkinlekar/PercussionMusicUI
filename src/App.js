...
const App = () => {
  ...
  const logout = () => {
    Promise.resolve(authctx.logout())
    .then(()=>{
        ...
        <ErrorBoundary>
         <Fragment className="content">
          <Switch>
            ...
            <Route path="/signin" exact component={Signin} /> {/* by assigning component this way we can get access to match location and history props. */}
...
//  That said, the chances of you making a mistake in writing setState such that batching ignores a change or sets incorrect value are high
//  (for instance you may call setState twice for the same key based on the previous value and might expect a result different from what you get). 
//  Hence its recommended that you call setState once after processing all the values